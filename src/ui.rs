...
pub fn render_pane(
    ...
    chunk: Rect,
    items: &[(String, Option<fs::Permissions>, bool)],
    state: &mut ListState,
    pane_type: PaneType
) {
    let list_items: Vec<ListItem> = items.iter().enumerate().map(|(index, (name, permissions, is_directory))| {
        let item_content = match pane_type {
            PaneType::Middle => {
                if let Some(permissions) = permissions {
                    let perms_str = fs_utils::get_permissions(&permissions);
                    format!("{:<width$} {}", name, perms_str, width = chunk.width as usize - perms_str.len() - 4)
                } else {
                    name.to_string()
                }
            }
        ...
        let normal_style = if *is_directory {
        ...
        let selected_style = if *is_directory {
